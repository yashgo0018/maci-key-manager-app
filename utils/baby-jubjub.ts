import { BigNumber, conversions, F1Field, scalar } from "@zk-kit/utils";
import * as sqrt from "./sqrt";
console.log(conversions);

const { leBigIntToBuffer, leBufferToBigInt } = conversions;
...
export function addPoint(p1: Point<bigint>, p2: Point<bigint>): Point<bigint> {
  ...
  // y3 = (y1*y2 - a*x1*x2)/(1 - d*x1*x2*y1*y2)
  const p3y = Fr.div(
    Fr.add(delta, Fr.sub(Fr.mul(a, beta), gamma)),
    Fr.sub(Fr.one, dtau)
  );
...
export function inCurve(p: Point): boolean {
  ...
  return Fr.eq(
    Fr.add(Fr.mul(a, x2), y2),
    Fr.add(Fr.one, Fr.mul(Fr.mul(x2, y2), d))
  );
}
...
export function unpackPoint(packedPoint: bigint): Point<bigint> | null {
  ...
  let x = sqrt.tonelliShanks(
    Fr.div(Fr.sub(Fr.one, y2), Fr.sub(a, Fr.mul(d, y2))),
    r
  );
